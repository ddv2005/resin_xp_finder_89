...
;		((((Anycubic Photon config file, compiled by Toby1kanobi))))
...
;	This setting allows you to replace the default leadscrew with other leadscrews that may have different thread pitches, a finer pitch will increase the machine's effective Z resolution. You must alter the Steps per mm acordingly
;
;
...
;(Z-axis Hold)
;M8016	D10
;	The unit is ms (milliseconds) Time build plate will spend stationiary between rise and lower operations
;
;
;
;(Max Z height mm)
...
;	Changing this value can allow you to shorten or lengthen the max Z volume. You may use this to install a longer Z axis
;
;
;
...
;	2: Double limit, if Z+ and Z- limit are used at the same time, Z+ can limit the maximum stroke of Z
;
;
...
;M8087	I0 T0	
;	I: Direction signal is valid to the establishment time of pulse signal high level, the unit is ns
;	T: The minimum holding time of the pulse signal in ns. If there is no external drive control board, set both to zero.
